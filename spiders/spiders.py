...
class IPCSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        url = 'https://www.secmca.org/params//?cid=3&scid=1&data=ITCER&parent=Tipos%20de%20cambio&son=%C3%8Dndice%20tipo%20de%20cambio%20efectivo%20real&list'
        yield SeleniumRequest(url=url, callback = self.parse, wait_time=35)
    
    def parse(self, response):
        ### selenium part ###
        # driver
        driver =response.meta['driver']
        opciones = ['Importaciones CIF', 'Exportaciones FOB', 'Remesas familiares: ingresos, egresos y neto', 'Ingresos totales: corrientes y de capital', 'Gastos totales: corrientes y de capital, Deuda pública mensual interna y externa y su relación con el PIB']
 
        # scraping all
        try: 
            WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, "//div[@class='button-box']/button"))).click() #select all countries
            ...
            if any(word == WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, '//div[@id="parameters"]/h2'))).text for word in opciones):
                WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, '//*[@id="params-form"]/div/div[1]/div[2]/div/div[2]/div[1]/label'))).click() #select first variable
                ...
                WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, '//*[@id="params-form"]/div/div[1]/div[2]/div/div[2]/div[1]/label'))).click() #select first variable
                WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, '//*[@id="params-form"]/div/div[1]/div[2]/div/div[2]/div[2]/label'))).click() #select second variable

            else:
                WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, '//div[@id="params-form"]/div/div[1]/div[2]/div/div[3]/button[1]'))).click() #select all variables
            
            sleep(2)
            WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, "//select[@id='extra-year-first']/option[@value='2017']"))).click() #select begining year
        except:
            try: 
                # the same from above only with refreshing and sleeping time 
                driver.refresh()
                sleep(35)
                WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, "//div[@class='button-box']/button"))).click()

                if any(word == WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, '//div[@id="parameters"]/h2'))).text for word in ['Importaciones CIF', 'Exportaciones FOB', 'Remesas familiares: ingresos, egresos y neto', 'Ingresos totales: corrientes y de capital', 'Gastos totales: corrientes y de capital']):
                    WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, '//*[@id="params-form"]/div/div[1]/div[2]/div/div[2]/div[1]/label'))).click() #select first variable
                ...
                # removing variables in case theres only one to select
                try:
                    driver.refresh()
                    sleep(35)
                    WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, "//div[@class='button-box']/button"))).click()
        ...
        # imae variables
        try:
            # WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, '//*[@id="params-form"]/div/div[1]/div[7]/div/div[3]/button[1]'))).click()
            WebDriverWait(driver,5).until(EC.presence_of_element_located((By.XPATH, '//*[@id="params-form"]/div/div[1]/div[7]/div/div[2]/div[2]'))).click()
        except:
            pass
        ...
        titulos = [tit.extract() for tit in response.xpath('//th[@class="text-center p-2 test"]/text()')] 
        titles = [ titulo +'-'+ var for titulo in titulos for var in variables]
        ...
        # to dataframe
        df = pd.DataFrame(contenido, index = fechas, columns=titles)
        print(df)
